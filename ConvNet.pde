...
void setup() {
  yeet = new ConvNetwork("k2.3.10,p2,k1.3.3,f1", 0.4);
  in = loadImage("in.jpg");
  size(768, 768);
}

void draw() {
  background((float) yeet.think(in)[0]*255);
  image(in, 0, 0);
  double[] one = {1};
  //yeet.learn(in, one, 0.01);
  println( yeet.think(in)[0]);
}
...
class Kernal {
  ...
  CLayer convolve(CLayer in, float S) {
    CLayer out = new CLayer(floor(in.W/S), floor(in.H/S), 1);
    for (int i = 0; i < in.W - S; i+=S) {
      for (int j = 0; j < in.H - S; j+=S) {
        out.image[floor(i/S)][floor(j/S)][0] = operate(in, i, j);
      }
    }
    return out;
  }
  
  void convolveLearn(CLayer in, CLayer error, float S, float lr) {
    for (int i = 0; i < in.W - S; i+=S) {
      for (int j = 0; j < in.H - S; j+=S) {
...
class ConvNetwork {
  ...
  */
  ConvNetwork(String in, float random) {
    ...
    strides = new float[coms.length];
    int krt = 0;
    for (int i = 1; i<coms.length; i++) {
      if (coms[i].charAt(0) == 'k') {
        locals[i] = krt;
        operations[i] = 0;
        strides[i] = float(split(coms[i],'.')[2]);
        krt++;
    ...
    for (int i = 1; i<operations.length; i++) {
      if (operations[i] == 0) { //kernal
        ...
        for (int j = 0; j<stacks.length; j++) {
           stacks[j] = kernals[locals[i]][j].convolve(layer[i - 1], strides[i]);
        }
  ...
  void learn(PImage in, double[] out, float lr) {
    ...
    for (int i = layerError.length - 3; i>0; i--) {
      if (operations[i] == 0) { //kernal
        CLayer[] stacks = new CLayer[kernals[locals[i]].length];
        for (int j = 0; j<stacks.length; j++) {
           stacks[j] = kernals[locals[i]][j].convolve(layerError[i + 1], 1/strides[i]);
        }
    ...
    for (int i = layerError.length - 2; i>0; i--) {
      for (int j = 0; j<kernals[locals[i]].length; j++) {
         kernals[locals[i]][j].convolveLearn(layer[i], layerError[i], strides[i], lr);
      }
    }
  }
